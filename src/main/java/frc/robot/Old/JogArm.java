...
public class JogArm extends Command {
  ...
  public void execute() {
     ...
     boolean allowDown = m_arm.getMotorEncoderAngleRadians() >= m_arm.armMin.in(Radians)
         || m_controller.getHID().getBackButton();

     boolean allowUp = m_arm.getMotorEncoderAngleRadians() <= m_arm.maxAngle.in(Radians)
         || m_controller.getHID().getBackButton();

    double yval = -m_controller.getRightY() / 10;
